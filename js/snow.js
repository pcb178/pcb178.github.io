/**
 * 雪花特效 + 爆炸碎片 + 积雪 + 主题色 + 点击音效
 * 保存为：source/js/snow.js
 */
...
(function () {
  ...
  const maxFlakes = 120; // ✅ 修复：定义缺失的 maxFlakes
  let mouseX = 0, mouseY = 0;

  /* ---------- 工具 ---------- */
  const isDark = () => document.documentElement.getAttribute('data-theme') === 'dark';
  const snowColor = () => isDark() ? '#ffffff' : '#6d7c94';
  ...
  function star(cx, cy, r, rot) {
    ...
    ctx.beginPath();
    for (let i = 0; i < 12; i++) {
      ctx.lineTo(cx + Math.cos(angle) * (i % 2 === 0 ? r : inner),
                 cy + Math.sin(angle) * (i % 2 === 0 ? r : inner));
      angle += step;
    }
    ctx.closePath();
  }
  ...
  class Snowflake {
    ...
    update() {
      ...
      if (this.y + this.r >= canvas.height - snowHeap[idx]) {
  ...
  class Fragment {
    ...
    update() { this.x += this.vx; this.y += this.vy; this.vy += this.gravity; this.alpha -= 0.03; }
    draw() {
      ctx.save();
      ctx.globalAlpha = this.alpha;
      ctx.fillStyle = snowColor();
  ...
  function drawHeap() {
    ...
    let total = 0;
    for (const h of snowHeap) total += h;
